...
## Quality floor — hard rules the output MUST pass

These are non-negotiable. If any output would violate one of these, revise before returning. No "artistic" exceptions.

**Contrast (WCAG AA is the floor, not the ceiling):**
- Body text : background ≥ 4.5:1. Large text (>= 18px bold or >= 24px regular) ≥ 3:1.
- **NEVER** dark ink on dark background. **NEVER** light ink on light background.
- If the palette uses a dark `--paper`, `--ink` MUST be a light color (>= 0.55 relative luminance). If `--paper` is light, `--ink` MUST be dark (<= 0.30 luminance).
- CTA text on button background: ≥ 4.5:1. If unsure, add a solid button surface, don't leave it translucent.
- Text over images ALWAYS needs a scrim (linear-gradient overlay) or a solid card background. Never lay text directly over a photo.
- Focus rings: 3:1 against the element AND against the background.

**Hierarchy (one thing wins per screen):**
- Exactly one dominant element per section. Three "important" things at the same weight = zero important things.
- Reading order must be enforced by size + contrast + isolation, in that priority order. If your headline is only bigger, it may still not read first — check its contrast is also higher.
- Whitespace above/around a hero element must be ≥ 1.5× the whitespace between its siblings.

**Spacing rhythm:**
- Every dimension from the 4/8/12/16/24/32/48/64/96/128 scale. No 13px, no 27px.
- Internal group gaps < between-group gaps, ratio ~1:2 minimum.
- Section vertical padding ≥ 64px on desktop; ≥ 96px for editorial / luxury.

**Typography:**
- One display family + one body family (mono third only when technical content is present).
- Body copy line-length 45–75 characters.
- Weight jumps: 400 → 500 → 600 → 700 (skip a step, don't stagger).
- Negative letter-spacing on display sizes (>= 32px), zero-to-positive on small text.

**Interactive states — always defined:**
- hover, focus-visible, active, disabled — every button, every link. If one is missing, add it.
- Focus ring must be visible on both light and dark backgrounds — use `outline: 2px solid var(--accent); outline-offset: 2px;` or equivalent.
- `cursor: pointer` on every clickable non-anchor.

**Images:**
- Reserve aspect ratio in CSS so the layout doesn't shift on load.
- Feature-cards use consistent aspect ratios across peers.
- Photos over text areas need a `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.15))` overlay minimum.
- `alt` on every `<img>` — describe the subject, not "image" or "photo".

**One signature element:**
- Exactly ONE memorable moment per page. Not two, not zero.
- It must feel drawn from the subject's world (a matrimony page's signature ≠ a fintech page's signature).
- If the reference sites suggest a signature (marquee, animated gradient, badge, 3D orb), pick ONE and commit.

**Anti-tells (the design will look AI-generated if you use these):**
- Cream + serif + terracotta (unless the subject is a bakery / editorial magazine — then earn it).
- Purple-to-blue gradient hero (the single most overused device).
- Unmotivated glassmorphism (frosted card floating in nothing).
- Near-black background + acid-green accent.
- "Get started" as the primary CTA (write a specific verb-noun instead: "Book 20 min", "Import invoices").
- Three feature cards with icons at the top, headline, one-line description — this is 60% of AI landing pages.
- Round-number stats ("10x faster", "99.9% uptime"). Use odd, specific numbers.
- Testimonials without a real-looking name + role + company + specific outcome.

**When copy is filled by another agent (placeholder-fill LLM):**
- Every placeholder must land specific to the subject, not to "SaaS in general."
- If the subject is "matrimony landing page", the words are wedding-specific, not "streamline your workflow."

If any of the above fails in your draft, revise before outputting. These rules override everything else in this KB.
